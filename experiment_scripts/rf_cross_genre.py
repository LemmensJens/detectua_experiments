...
for genre in tqdm(df["genre"].unique()):
    ...
    test_split = df[df["genre"] == genre]
    train_split = df[df["genre"] != genre]
    ...
    # -------------------------
    pipeline = Pipeline([
        ("vec", TfidfVectorizer(ngram_range=(1, 1), lowercase=True)),
    ...
    metrics_df.to_csv(os.path.join(args.output, f"cv_results_no_{genre}.csv"), index=False)
    ...
    # Results on human class
    p_h, r_h, f_h, s_h = precision_recall_fscore_support(y_test, y_test_pred, average="binary", pos_label=0)
...
results_df.to_csv(os.path.join(args.output, "cross_genre_test_classification_report.csv"), index=False)