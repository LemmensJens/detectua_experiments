...
# =========================
# BEGIN CROSS-DATASET EVALUATION
# ========================
for dataset in tqdm(df["corpus"].unique()):
    print("\n===OUT-OF-DOMAIN DATASET:", dataset, "===\n")

    # DATA SPLIT
    train_val_df, test_df = df[df["corpus"] != dataset], df[df["corpus"] == dataset]

    # To ensure a representative validation set, we stratify on both label and corpus
    stratify_cols = train_val_df[["binary_label", "corpus"]]
    ...
    for seed in seeds:
        ...
        for lr in learning_rates:
            ...
            run_name = f"Test{dataset}_seed{seed}_lr{lr}"

            wandb.init(
                project="robbert-ai-detection-cross-dataset",
                name=run_name,
                config={
                    "seed": seed,
                    "learning_rate": lr
        ...
        with open(os.path.join(args.output, f"{dataset}_best_params_seed{seed}.txt"), "w") as f:
        ...
        run_name = f"{dataset}_inference_seed_{seed}"
        ...
        test_out.to_csv(
            os.path.join(args.output, f"{dataset}_test_predictions_seed{seed}.csv"),
            index=False
        )
