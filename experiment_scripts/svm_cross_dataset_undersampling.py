...
for dataset in tqdm(df["corpus"].unique()):
    ...
    X_train_ros, y_train_ros = ros.fit_resample(
        train_split[["text"]],
        train_split["binary_label"]
    )

    train_split = X_train_ros.copy()
    train_split["binary_label"] = y_train_ros
    ...
    label_dist_ros = train_split["binary_label"].value_counts()
    print("Label distribution after oversampling:")
    print(label_dist_ros)
